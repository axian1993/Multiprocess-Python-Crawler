    ...
    def get_all_answers(self):
        ...
        if answers_num == 0:
            ...
            for i in range((answers_num - 1) / 20 + 1):
                if i == 0:
                    for j in range(min(answers_num, 20)):
                        if self.soup == None:
                            self.parser()
                    ...
                    r = self.requests.post(post_url, data=data, headers=header)

                    answer_list = r.json()["msg"]
                    for j in range(min(answers_num - i * 20, 20)):
                        soup = BeautifulSoup(self.soup.encode("utf-8"))
...
class User:
    ...
    def get_followees(self):
        if self.user_url == None:
            ...
            if followees_num == 0:
                ...
                r = self.requests.get(followee_url)
    ...
    def get_followers(self):
        if self.user_url == None:
            ...
            if followers_num == 0:
                ...
                r = self.requests.get(follower_url)
        ...
        if self.user_url == None:
            ...
            if answers_num == 0:
                ...
                for i in range((answers_num - 1) / 20 + 1):
                    ...
                    r = self.requests.get(answer_url)
                    soup = BeautifulSoup(r.content)
        ...
        if self.user_url == None:
            ...
            if collections_num == 0:
                ...
                for i in range((collections_num - 1) / 20 + 1):
                    ...
                    r = self.requests.get(collection_url)
    ...
    def get_likes(self):
        ...
        if self.user_url == None:
            ...
            r = self.requests.get(self.user_url)
            soup = BeautifulSoup(r.content)
    ...
    def get_activities(self, proxy = None):
        if self.user_url == None:
            ...
            def yield_activeity(activity):
                ...
                if _type == 'a':
                    answer_url = zhihu_url + activity.find("a", class_ = 'question_link')['href']
                    return json.dumps({'timestamp': timestamp, 'type': _type, 'type_detail': type_detail, 'answer_url': answer_url})

                elif _type == 'p':
                    post_url = zhihu_url + activity.find("a", class_ = 'post-link')['href']
                    ...
                    if type_detail == 'member_ask_question' or type_detail == 'member_follow_question':
                        question_url = zhihu_url + activity.find("a", class_ = 'question_link')['href']
                        ...
                        column_url = activity.find("a", class_ = 'column_link')['href']
                        ...
                        topic_url = zhihu_url + activity.find("a", class_ = 'topic-link')['href']
                        ...
                        roundtable_url = zhihu_url + activity.find("a", class_ = 'roundtable_link')['href']
...
class Answer:
    ...
    def parser(self):
        ...
        try:
            ...
            Logging.info("执行 `python auth.py` 即可以完成登录。")
            raise Exception("无权限(403)")
        r = self.requests.get(self.answer_url)
        soup = BeautifulSoup(r.content)
        self.soup = soup
    ...
    def get_voters(self):
        ...
        r = self.requests.get(request_url, params={"params": "{\"answer_id\":\"%d\"}" % int(data_aid)})
        soup = BeautifulSoup(r.content)
        ...
        if len(voters_info) == 0:
            ...
            for voter_info in voters_info:
                if voter_info.string == "匿名用户、" or voter_info.string == "匿名用户":
                    voter_url = None
...
class Collection:
    ...
    def get_all_answers(self):
        ...
        if len(answer_list) == 0:
            ...
            i = 2
            while True:
                r = self.requests.get(self.url + "?page=" + str(i))
