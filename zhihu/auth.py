...
class LoginPasswordError(Exception):
    def __init__(self, message):
        if type(message) != type("") or message == "": self.message = "帐号密码错误"
        else: self.message = message
        Logging.error(self.message)

class NetworkError(Exception):
    def __init__(self, message):
        if type(message) != type("") or message == "": self.message = "网络异常"
        else: self.message = message
        Logging.error(self.message)
class AccountError(Exception):
    def __init__(self, message):
        if type(message) != type("") or message == "": self.message = "帐号类型错误"
        else: self.message = message
        Logging.error(self.message)
...
def upload_form(form):
    ...
    if r.headers['content-type'].lower() == "application/json":
        ...
        if result["r"] == 0:
            ...
            return {"error": {"code": -1, "message": "unknow error"} }
    else:
